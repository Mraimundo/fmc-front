...
export default (props: Props) => {
  ...
  const revenuesGoal = format(revenues.goalInKilosByLiter, 0, 0);
  const revenuesRealized = format(revenues.realizedInKilosByLiter, 0, 0);
  const pogGoal = format(pog.goalInKilosByLiter, 0, 0);
  const pogRealized = format(pog.realizedInKilosByLiter, 0, 0);
